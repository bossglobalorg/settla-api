...
describe('UserService', () => {
  ...
  it('should be able to create user', async () => {
    const passwordSpy = jest.spyOn(passwordService, 'generate').mockResolvedValue('password-hash')
    const jwtSpy = jest.spyOn(jwtService, 'sign').mockReturnValue('jwt')
    const createSpy = jest.spyOn(repo, 'create').mockReturnValue(mockUserEntity)
  ...
  it('should check user password', async () => {
    const compareSpy = jest.spyOn(passwordService, 'compare').mockResolvedValue(true)

    expect(await service.checkUserPassword(mockUserEntity, 'request-password')).toBe(true)
    expect(compareSpy).toHaveBeenCalledWith('request-password', mockUserEntity.passwordHash)
  })

  it('should get all users', async () => {
    const repoSpy = jest.spyOn(repo, 'find').mockResolvedValue([mockUserEntity])
